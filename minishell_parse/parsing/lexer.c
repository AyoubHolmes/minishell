...
void	ft_lexer_token_helper(char *c, char *line, int setter, int token)
{
	if (token == SINGLE_QUOTE_TOKEN || token == DOUBLE_QUOTE_TOKEN)
	{
		...
		*c |= setter;
	*line = token;
	if (*line == REDIRECTION3_TOKEN && *(line + 1) == '>'
...
int	ft_error_checker(char *c, char *line, t_minishell *cli)
{
	...
	if ((*line == '|' || *line == ';' || (*line == '\\' && *(line + 1) == '\0'))
		...
		(*c & REDIRECTION3_SETTER && *line == REDIRECTION3_TOKEN
				&& *(line - 1) == REDIRECTION3_TOKEN && *(line + 1) == '\0')))
		return (4);
...
{
	if ((cli->c & REDIRECTION1_SETTER) != 0
		|| ((cli->c & REDIRECTION2_SETTER) == 0
			&& (cli->c & REDIRECTION3_SETTER) == 0 && *(cli->helper) == '>'))
	{
	...
	else if ((cli->c & REDIRECTION2_SETTER) != 0
		|| ((cli->c & REDIRECTION1_SETTER) == 0
			&& (cli->c & REDIRECTION3_SETTER) == 0 && *(cli->helper) == '<'))
	{
